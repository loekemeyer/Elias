...
async function refreshAuthState(sessionOverride) {
  ...
  // Panel Admin: visible para cualquier admin.
  var codAdminGate = String(customerProfile?.cod_cliente || "").trim();
  var isLoekemeyerAdmin = isAdmin;
  if ($("menuAdminPanel"))
...
function waLink(msg) {
  ...
  return `https://wa.me/5491131181185?text=${text}`;
}
...
function renderProducts() {
  ...
  cats.forEach((category) => {
    ...
    if (String(category).trim().toLowerCase() === "cuadros") {
